 ...
 * 
 */
function inicioPanel(panelId, panelLabel, i, panel, id, manutencao) {
...
function montaCabecalho(header, id, manutencao) {

	$.each(header, 
			function(i, header) {
		var labelId = header.label.replace( /\s/g, '' ) + 1 + "-" + i;
		montaCampos(i, "cabecalho", 1, header, "cabecalho", id, manutencao)
	});
};

function finalPanel(panelId, panelLabel, i, panel, manutencao) {
	$("#paineis").append(
			'</table>' +
		'</div>' +
		'<!-- ' + panel.label + ' -->' +
...
function montaCampos(i, panelId, z, item, origem, id, manutencao) {
	...
	$("#table-" + panelId).append(
			'<ul id="tr-' + labelId + '" data-role="listview" data-inset="true">' +
				'<li id="td-textinput-' + labelId + '" class="ui-field-contain">' + 
					' <label "textinput-' + labelId + '">' + label + '</label>'
	);
